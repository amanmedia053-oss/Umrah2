...
const BottomNav = ({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (t: string) => void }) => {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-40 border-t pb-[env(safe-area-inset-bottom)] ${isDarkMode ? 'bg-black border-white/10' : 'bg-white border-black/5'}`}>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 relative z-10">  
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isDarkMode ? 'bg-black' : 'bg-gray-50'}`}>  
    ...
    <AnimatePresence>
      {isPlayerOpen && (
        ...
        >
          ...
          <div className="pt-[max(env(safe-area-inset-top),1.5rem)] px-6 pb-4 flex justify-between items-center relative z-10">  
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-8 pb-[env(safe-area-inset-bottom)] relative z-10">  
            <div className={`w-64 h-64 rounded-full border-4 flex items-center justify-center relative ${isDarkMode ? 'border-white/10' : 'border-black/5'}`}>  
              <div className={`absolute inset-4 rounded-full border border-dashed opacity-20 animate-spin-slow ${isDarkMode ? 'border-white' : 'border-black'}`} />  
              <div className={`w-48 h-48 rounded-full flex items-center justify-center overflow-hidden ${getAccentClass(accentColor)}`}>  
            ...
            <div className="w-full mt-12">  
              <input  
                ...
                className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-current ${isDarkMode ? 'bg-white/10' : 'bg-black/5'}`}  
            ...
            <div className="flex items-center justify-center gap-8 mt-12">  
              ...
              <button   
                onClick={togglePlay}  
                className={`w-20 h-20 rounded-full flex items-center justify-center shadow-lg transition-transform active:scale-95 ${isDarkMode ? 'bg-white text-black' : 'bg-black text-white'}`}  
              >  
...
const HomeScreen = () => {
  ...
  return (
    <div className="pb-28">
      {/* هېډر چې تر سټاټس بار لاندې غځول شوی دی */}
      <div className={`px-6 pb-6 pt-[max(env(safe-area-inset-top),2.5rem)] rounded-b-[40px] relative overflow-hidden mb-6 ${getAccentClass(accentColor)}`}>
...
const FavoritesScreen = () => {
  ...
  return (
    <div className="px-6 pt-[max(env(safe-area-inset-top),2.5rem)] pb-28">
    ...
    <div className="px-6 pt-[max(env(safe-area-inset-top),2.5rem)] pb-28">
      ...
      <SettingItem label="د شپې حالت">  
        <button   
          ...
          className={`w-12 h-6 rounded-full transition-colors relative ${isDarkMode ? 'bg-white' : 'bg-gray-200'}`}  
        >  
          <div className={`absolute top-1 w-4 h-4 rounded-full transition-all ${isDarkMode ? 'right-1 bg-black' : 'left-1 bg-white shadow-sm'}`} />  
        </button>  
      </SettingItem>  
      ...
      <SettingItem label="بل درس اوتومات شروع شي">  
        <button   
          ...
          className={`w-12 h-6 rounded-full transition-colors relative ${autoNext ? getAccentClass(accentColor) : 'bg-gray-200'}`}  
        >  
          <div className={`absolute top-1 w-4 h-4 rounded-full transition-all bg-white shadow-sm ${autoNext ? 'right-1' : 'left-1'}`} />  
        </button>  
      </SettingItem>  
        ...
        <div className="grid grid-cols-5 gap-3">  
          {colors.map(c => (  
            <button  
              ...
              className={`w-10 h-10 rounded-full border-2 transition-transform active:scale-90 ${accentColor === c.id ? 'border-white scale-110 shadow-lg' : 'border-transparent'}`}  
      ...
      <button className={`w-full p-4 rounded-2xl mb-3 flex items-center justify-center gap-2 font-bold ${isDarkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>  
        <Share2 size={20} />  
        اپ شريکول  
      </button>  
...
const AboutScreen = () => {
  ...
  return (
    <div className="px-6 pt-[max(env(safe-area-inset-top),2.5rem)] pb-28">
      ...
      <div className={`p-6 rounded-3xl mb-8 leading-relaxed text-justify ${isDarkMode ? 'bg-zinc-900 text-gray-300' : 'bg-white shadow-sm border border-black/5 text-gray-600'}`}>  
        <p className="mb-4">  
